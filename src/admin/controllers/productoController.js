...
const productoController = {
  ...
  async listar(req, res) {
    try {
        ...
        res.render('Admin/pages/productos/lista', {
  ...
  async crear(req, res) {
    try {
        const { nombre, precio_base, marca_id, descripcion_corta, descripcion_larga, sku, activo, destacado, categorias, etiquetas } = req.body;
        ...
        if (categorias && categorias.length > 0) {
            await ProductoService.assignCategories(nuevoProducto.id, Array.isArray(categorias) ? categorias : [categorias]);
        }

        if (etiquetas && etiquetas.length > 0) {
            await ProductoService.assignTags(nuevoProducto.id, Array.isArray(etiquetas) ? etiquetas : [etiquetas]);
        }
  ...
  async editar(req, res) {
    try {
        ...
        if (categorias) {
            await ProductoService.assignCategories(id, Array.isArray(categorias) ? categorias : [categorias]);
        }

        if (etiquetas) {
            await ProductoService.assignTags(id, Array.isArray(etiquetas) ? etiquetas : [etiquetas]);
        }
